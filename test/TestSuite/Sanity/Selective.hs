...
import Test.Falsify.Generator (Gen, run, shrink, shrinkWithShortcut, prim)
...
testShrinking gen propGen propHistory i
  ...
  | propHistory shrinkHistory = Just TestOk
  | otherwise                 = Just $ TestFailed shrinkHistory
  where
    st :: SampleTree
    st = SampleTree.map (`mod` 100) $ SampleTree.fromSeed i

    shrinkHistory :: [a]
    shrinkHistory = toList $ shrink (not . propGen) gen st
...
ifBoth :: Monad m => m Bool -> m a -> m a -> m a
ifBoth cond t f = do
    x <- t
    y <- f
    cond >>= \b -> return $ if b then x else y